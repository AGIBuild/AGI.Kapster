...
public static class TestDataFactory
{
    ...
    public static string CreateGitHubReleasesResponse(
        ...
        string assetName = "AGI.Captor-1.3.0-win-x64.msi",
        string downloadUrl = "https://github.com/AGIBuild/AGI.Captor/releases/download/v1.3.0/AGI.Captor-1.3.0-win-x64.msi",
        long assetSize = 10485760)
    {
        var release = new
        {
            tag_name = $"v{version}",
            name = $"AGI.Captor {version}",
            body = "Release notes for version " + version,
            published_at = DateTime.UtcNow.ToString("yyyy-MM-ddTHH:mm:ssZ"),
            prerelease = prerelease,
            html_url = $"https://github.com/AGIBuild/AGI.Captor/releases/tag/v{version}",
            assets = new[]
            {
                new
                {
    ...
    public static string CreateMultiPlatformGitHubReleasesResponse(string version = "1.3.0")
    {
        var release = new
        {
            ...
            assets = new[]
            {
                new
                {
                    name = $"AGI.Captor-{version}-win-x64.msi",
                    browser_download_url = $"https://github.com/AGIBuild/AGI.Captor/releases/download/v{version}/AGI.Captor-{version}-win-x64.msi",
                    size = 10485760L,
                    content_type = "application/octet-stream"
                },
                new
                {
                    name = $"AGI.Captor-{version}-osx-x64.pkg",
                    browser_download_url = $"https://github.com/AGIBuild/AGI.Captor/releases/download/v{version}/AGI.Captor-{version}-osx-x64.pkg",
                    size = 8388608L,
                    content_type = "application/octet-stream"
                },
                new
                {
                    name = $"AGI.Captor-{version}-linux-x64.deb",
                    browser_download_url = $"https://github.com/AGIBuild/AGI.Captor/releases/download/v{version}/AGI.Captor-{version}-linux-x64.deb",
                    size = 9437184L,
                    content_type = "application/octet-stream"
                }
            }
