...
public class ExportService : IExportService
{
    ...
    /// <summary>
    /// Convert Avalonia bitmap to SkiaSharp bitmap
    /// </summary>
    private async Task<SKBitmap> ConvertToSkiaBitmapAsync(Bitmap avaloniabitmap, ExportSettings settings)
    {
        return await Task.Run(() =>
        {
            try
            {
                ...
                {
                    var backgroundBitmap = new SKBitmap(skiaBitmap.Width, skiaBitmap.Height);
                ...
                try
                {
                    switch (settings.Format)
                    {
                        ...
                        case ExportFormat.JPEG:
                            encodedData = image.Encode(SKEncodedImageFormat.Jpeg, settings.Quality);
                            break;
                        ...
                        case ExportFormat.BMP:
                            // Use System.Drawing for BMP since SkiaSharp has issues
                            try
                            {
                                // First encode as PNG in memory
                                using (var tempPngData = image.Encode(SKEncodedImageFormat.Png, 100))
                                using (var tempStream = new MemoryStream(tempPngData.ToArray()))
                                {
                                    if (OperatingSystem.IsWindowsVersionAtLeast(6, 1))
                                    {
                                        // Use System.Drawing to convert PNG to BMP
                                        using (var tempBitmap = System.Drawing.Image.FromStream(tempStream))
                                        using (var bmpStream = new MemoryStream())
                                        {
                                            tempBitmap.Save(bmpStream, System.Drawing.Imaging.ImageFormat.Bmp);
                                            encodedData = SKData.CreateCopy(bmpStream.ToArray());
                                        }
                                    }
                                    else
                                    {
                                        // Fallback to PNG on non-Windows
                                        Log.Warning("BMP format only supported on Windows, saving as PNG");
                                        encodedData = tempPngData;
                                    }
                                }
                            }
                            catch (Exception bmpEx)
                            {
                                Log.Warning(bmpEx, "Failed to create BMP, using PNG fallback");
                                encodedData = image.Encode(SKEncodedImageFormat.Png, 100);
                            }
                            break;
