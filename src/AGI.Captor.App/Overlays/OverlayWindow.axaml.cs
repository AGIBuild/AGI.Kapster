...
public partial class OverlayWindow : Window
{
	...
	public OverlayWindow(IElementDetector? elementDetector = null)
	{
		...
		if (this.FindControl<SelectionOverlay>("Selector") is { } selector)
		{
			...
			selector.ConfirmRequested += r =>
			{
				try
				{
					...
					{
						var ok = CopyRegionToClipboardWindows(r);
						Log.Information("CopyRegionToClipboardWindows result={Ok}", ok);
					}
				}
				catch (Exception ex)
				{
		...
		if (this.FindControl<NewAnnotationToolbar>("Toolbar") is { } toolbar && existingAnnotator != null)
		{
			...
			// Then set default tool to Arrow (after toolbar setup to avoid conflicts)
			Log.Information("Setting default tool to Arrow after toolbar setup");
	...
	protected override void OnKeyDown(KeyEventArgs e)
	{
		...
		else if (e.Key == Key.Enter)
		{
			if (this.FindControl<SelectionOverlay>("Selector") is { } selector && selector is not null)
			{
				...
				if (r.Width > 0 && OperatingSystem.IsWindows())
				{
					try
					{
						var ok = CopyRegionToClipboardWindows(r);
        ...
        try
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    var screenshot = CaptureRegionWindows(selectionRect);
                    if (screenshot == null)
                    {
